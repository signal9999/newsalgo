...
2. **このファイル（newsalgo_knowledge_v5.md）をアップロード**してから作業開始
...
| Yahoo 株式 | `https://news.yahoo.co.jp/rss/topics/stock.xml` | 約 8 件 | 0.65 | ✅ |
| みんかぶ | `https://minkabu.jp/news/rss` | 数十件 | 0.70 | ✅ |
| TDnet 適時開示 | HTMLスクレイピング（平日 08:00〜15:35 JST） | 数件〜数十件 | 0.90 | ✅ |

**合計: 約 150+ 件/回（TDnet除く）**
...
# TWS を起動 → API → Settings → Enable ActiveX and Socket Clients = ON
# .env に追記:
#   IBKR_HOST=127.0.0.1
#   IBKR_PORT=7497   # paper trade
```
...
- [ ] SIGNAL_THRESHOLD を 0.7 に変更してペーパートレード精度向上
- [ ] バックテスト: LLM モード（`--llm`）での精度・コスト実測
- [ ] Slack/LINE: 実トークン設定・本番通知テスト
- [ ] Phase 3: IBKR ペーパートレード口座で実接続テスト
- [ ] スケジューラー: VPS/サーバーへのデプロイ（`pm2` または `systemd`）
- [ ] みんかぶ RSS: 取得件数・記事品質の実動作確認
