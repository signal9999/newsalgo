...
2. **このファイル（newsalgo_knowledge_v6.md）をアップロード**してから作業開始
...
- イメージ: `redis:7-alpine`
- 最大メモリ: 256 MB（LRU eviction）
- ポート: 6379
- ヘルスチェック付き

`.env` の `REDIS_URL=redis://localhost:6379` はそのまま使用可。
...
**サンプルデータ（2025-05-20〜24、5銘柄、hold=1）**
```
総トレード数:     5 件 / 勝率: 60.0% / 総損益: ¥2,712(+0.27%)
PF: 3.96 / 最大DD: -0.07% / シャープ: 8.60
```

**実 JSONL ログ（2025-05-20〜23、7銘柄、hold=1）**
```
総トレード数:     7 件 / 勝率: 57.1% / 総損益: ¥396(+0.04%)
PF: 1.18 / 最大DD: -0.14% / シャープ: 0.95
ソース別: tdnet 60.0% / nhk_economy 50.0%
```
...
| TDnet が JS レンダリング必要 | ✅ 解決済 | BeautifulSoup で HTML パース方式に変更（証券コード・企業名抽出も対応） |
...
| Redis が未起動 | ✅ 解決済 | `docker-compose.yml` 作成・`docker compose up -d redis` で起動 |
...
## 10. コマンドリファレンス（全バージョン）

```bash
# hold_days × threshold 全組み合わせ最適化
python3 run_optimize.py                  # サンプルデータ
python3 run_optimize.py --mode jsonl     # 実ログ
python3 run_optimize.py --csv            # CSV 出力

# LLM コスト試算
python3 run_backtest.py --cost           # API 呼出なし・試算のみ
python3 run_backtest.py --llm            # 実際に LLM を使ってバックテスト

# Phase B スケジューラー（GitHub Actions 不要）
python3 scheduler.py                     # 常時稼働スタート（Ctrl+C で停止）
python3 scheduler.py --dry-run           # スケジュール確認のみ
python3 scheduler.py --now               # 即時1回実行

# アラートテスト
python3 scripts/test_alert.py            # 全テスト送信
python3 scripts/test_alert.py --check    # 設定確認のみ

# Redis 起動（Docker/Homebrew/redis-server を自動判定）
...
### IBKR 接続手順（Phase 3 準備）

```bash
pip install ib_insync                    # ライブラリインストール
...
# TWS 設定: File → Global Configuration → API → Settings
#   Enable ActiveX and Socket Clients: ON / Socket port: 7497
```

### VPS デプロイ手順

```bash
# 新規 VPS (Ubuntu 22.04) へのセットアップ
bash deploy/setup_vps.sh

# Makefile によるショートカット
make test          # 全テスト
make backtest      # バックテスト
make optimize      # パラメータ最適化
make rss           # RSS 取得確認
make run           # スケジューラー起動

# Slack/LINE Secrets を GitHub に登録
SLACK_WEBHOOK_URL=https://... LINE_NOTIFY_TOKEN=xxx \
...
| モード | 件数 | 勝率 | PF | APIコスト |
|---|---|---|---|---|
| キーワードのみ | 6件 | 66.7% | 2.28 | $0 |
| LLM（claude-sonnet-4-5） | 6件 | 66.7% | 2.28 | $0.027（¥4.2） |

→ **精度は同等。LLM は 10件あたり ¥4.2 の追加コスト。**
...
- [ ] Phase 3: IBKR ペーパートレード口座で実接続テスト（`pip install ib_insync` 後）
- [ ] VPS デプロイ: `bash deploy/setup_vps.sh` を実サーバーで実行
- [ ] バックテスト: 実 JSONL ログが蓄積されたら精度評価（`--mode jsonl`）
- [ ] LLM モードの本番適用検討（コスト vs 精度トレードオフ）
